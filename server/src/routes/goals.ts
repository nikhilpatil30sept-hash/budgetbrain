 ...
 */
function weeklyVelocityCents(): number {
 ...
 */
function goalStats(goal: GoalRow, velocity: number) {
  const today = todayISO();
  ...
  const { net } = db
    .prepare(
      `SELECT COALESCE(SUM(amount_cents), 0) AS net
       FROM transactions
       WHERE date >= ? AND category != 'Transfers'`
    )
    .get(createdDate) as { net: number };
...
goalsRouter.get("/", (_req, res) => {
  const goals = db.prepare("SELECT * FROM goals ORDER BY deadline ASC").all() as GoalRow[];
  const velocity = weeklyVelocityCents();
  res.json({ velocity_weekly_cents: velocity, goals: goals.map((g) => goalStats(g, velocity)) });
});
...
goalsRouter.post("/", (req, res) => {
  ...
  const info = db
    .prepare("INSERT INTO goals (name, target_cents, deadline, created_at) VALUES (?, ?, ?, ?)")
    .run(name, target_cents, deadline, nowISO());
  const goal = db.prepare("SELECT * FROM goals WHERE id = ?").get(info.lastInsertRowid) as GoalRow;
  res.status(201).json(goalStats(goal, weeklyVelocityCents()));
});
...
goalsRouter.patch("/:id", (req, res) => {
  ...
  res.json(goalStats(goal, weeklyVelocityCents()));
});
