...
// A tight ceiling in production (auth endpoints are a favorite brute-force
// target), but a much more generous one everywhere else — local dev and the
// e2e suite both log in far more than 20 times per 15 minutes just by
// existing (every spec logs in at least once), and that has nothing to do
// with the abuse this limit exists to stop.
