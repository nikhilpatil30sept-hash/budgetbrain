...
- **Always review the "what we'll send to the AI" preview before trusting it.** The redaction targets long (8+ character) number runs — including bank-style asterisk-masked references — SIN/SSN shapes, emails, and phone numbers, plus whole lines that look like a repeated per-page identity header (a line starting with a salutation, or containing an `XXXX`-style masked card number). This preview step exists precisely so a human confirms nothing sensitive slipped through; don't treat auto-redaction as infallible. It has already caught real gaps once: a manual pass against a real statement found the header-crop missing a repeated "MR NAME – masked card" line after the first page, and separately found asterisk-masked account references in transaction lines (e.g. a "payment from" entry) going completely unredacted — both fixed, but the lesson stands: test every new bank's statement format against this preview before trusting it, since an unfamiliar layout could still slip something through.
